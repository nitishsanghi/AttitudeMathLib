...
namespace AML{
    const Matrix33 rotationX(double theta){
    ...
    const Matrix33 rotationY(double theta){
    ...
    const Matrix33 rotationZ(double theta){
        double data[3][3] = {{cos(theta), sin(theta), 0.0},
    ...
    bool isValidDCM(const Matrix33& dcm, double tol = std::numeric_limits<double>::epsilon()){
        ...
        // Test Determinant
        bool det_test = fabs(determinant(dcm) - 1) < tol_limit;
